...
Working only in ``dev`` will inevitably cause conflicts, which will slow down development if they occur often. For this reason, whenever you want to start working on a task, you create a branch from the latest commit of ``dev``.
...
You then continue to work on that branch for that specific task and once you are done with your task, you pull request into ``dev``. The pull requests' name is the branch's name but capitalized and with spaces. It is optional to add reviewers, assignees or labels.

![[Pasted image 20241126224748.png]]

Once you have created the pull request, it will check if there are any merge conflicts which will allow you to resolve them. After that is done or when you haven't found any merge conflicts, merge the branch to ``dev``. Should you wish to continue working on the task, you can leave your branch alive. Otherwise, it is best to delete the branch once you are done.